...
export default function HistorialView({ invoiceHistory, setInvoiceHistory, loadInvoiceForEdit, downloadFromHistory, deleteInvoice, pdfBusy, exportImeiExcel }) {
  const s = styles;
  return (
              ...
              {invoiceHistory.map((h, i) => (
                <tr key={i}>
                  ...
                  <td style={{ ...s.invTd, textAlign: "left", whiteSpace: "nowrap" }}>
                    ...
                    {h.type === "factura" && exportImeiExcel && (
                      <><button onClick={() => exportImeiExcel(h)} style={{ ...s.miniBtn, borderColor: "#2f6d4a", color: "#8ee0a8" }} title="Bajar Excel .xlsx con PRODUCTO / MODELO / Nº de serie (una fila por unidad)">⬇ Excel x unidad</button>{" "}</>
                    )}
