...
export const SUPERVISOR_MODEL = "gemini-2.5-pro"; // supervisor: modelo más inteligente que revisa/aprende

// fetch con timeout: si una llamada al modelo queda colgada (Pro es lento / red), no deja
// el agente pensando para siempre — corta y tira un error claro.
async function fetchT(url, opts, ms = 90000) {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), ms);
  try { return await fetch(url, { ...opts, signal: ctrl.signal }); }
  catch (e) { if (e && e.name === "AbortError") throw new Error("Timeout: la consulta tardó demasiado. Probá de nuevo o apagá 🧠 modo inteligente (Pro es más lento)."); throw e; }
  finally { clearTimeout(t); }
}

// Shared disambiguation rules: section headers (EURO/LATIN) + base-variant hint.
const DISAMBIG =
  "If the text has section headers like EURO or LATIN, use them to disambiguate (EURO = the models ending in 'EURO', or codes like 'XT2xxx ...'; LATIN = the 'Motorola ...' models without 'EURO'). A bare base name like 'Edge 60' (no Neo/Fusion/Pro) means the base variant of that section.";
...
export function buildParseSystem(lines) {
  return (
    ...
    lines.join("\n") +
    "\n\nRules:\n- Ignore colors for Android phones (Samsung/Motorola) — color does NOT change their price.\n" +
    "- EXCEPTION — APPLE / iPhone: color DOES change the price. For Apple/iPhone products, treat EACH COLOR as a SEPARATE model and INCLUDE the color in the name (e.g. 'iPhone 17 Pro Max 256GB Orange', 'iPhone 17 Pro Max 256GB Silver'), each with ITS OWN price. Set \"dept\":\"iPhone\" and \"cat\":\"iPhone\" for these. Keep a region hint if present (e.g. 'US').\n" +
    ...
    "\n- For models that clearly do NOT match any SKU in the list, do NOT force them. Put them under \"new\" with a normalized name in the SAME naming style, a \"cat\" (Samsung/Motorola LATIN/Motorola EURO for Android; a free label like 'iPhone' or 'MacBook' otherwise), an optional \"dept\" (default 'Teléfonos'; 'iPhone' for Apple; 'Laptops' for laptops), and the price.\n" +
    'Respond ONLY with JSON: {"matched": {"<exact SKU>": price, ...}, "tiers": {"<exact SKU>": [{"min":1,"price":N},{"min":20,"price":N}], ...}, "new": [{"name": "...", "cat": "...", "dept": "...", "price": N}, ...]}. No markdown, no commentary.'
  );
}
...
export const toNum = (v) => {
  ...
  let s = v.replace(/[^0-9.,]/g, ""); // dejar dígitos, punto y coma
  if (!s) return null;
  if (s.includes(".") && s.includes(",")) {
    // ambos separadores: el ÚLTIMO es el decimal (1.105,00 → 1105.00 ; 1,105.00 → 1105.00)
    if (s.lastIndexOf(",") > s.lastIndexOf(".")) s = s.replace(/\./g, "").replace(",", ".");
    else s = s.replace(/,/g, "");
  } else if (s.includes(",")) {
    // solo coma: si es ,NN al final es decimal; si no, separador de miles
...
export async function callGemini({ system, content, apiKey, maxTokens = 2048, json = false, images = [], model = GEMINI_MODEL }) {
  let data;
  if (import.meta.env.DEV) {
    const parts = [];
    for (const im of images) parts.push({ inline_data: { mime_type: im.mimeType, data: im.data } });
    if (content) parts.push({ text: content });
    const body = { contents: [{ role: "user", parts }], generationConfig: { temperature: 0, maxOutputTokens: maxTokens, ...(/flash/i.test(model) ? { thinkingConfig: { thinkingBudget: 0 } } : {}) } };
    ...
    const res = await fetchT("/api/gemini", {
      method: "POST",
      headers: { "content-type": "application/json", "x-app-password": apiKey || "" },
      body: JSON.stringify({ system, content, images, json, maxTokens, model }),
    });
    if (!res.ok) {
      let detail = "";
...
export async function callGeminiTools({ system, contents, tools, apiKey, maxTokens = 2048, model = GEMINI_MODEL }) {
  let data;
  if (import.meta.env.DEV) {
    const body = { contents, tools, generationConfig: { temperature: 0, maxOutputTokens: maxTokens, ...(/flash/i.test(model) ? { thinkingConfig: { thinkingBudget: 0 } } : {}) } };
    if (system) body.system_instruction = { parts: [{ text: system }] };
    const res = await fetchT(
      `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(apiKey)}`,
      { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) }
    ...
    const res = await fetchT("/api/gemini", {
      method: "POST",
      headers: { "content-type": "application/json", "x-app-password": apiKey || "" },
      body: JSON.stringify({ system, contents, tools, maxTokens, model }),
    });
...
export async function parseSupplierQuote(rawText, apiKey, system, names, images = []) {
  ...
  const newModels = (Array.isArray(parsed.new) ? parsed.new : [])
    .map((m) => ({ name: String(m?.name || "").trim(), cat: m?.cat || "", dept: m?.dept ? String(m.dept).trim() : "", price: toNum(m?.price) }))
    .filter((m) => m.name && !known.has(m.name));
  // rescate: entradas que el modelo puso en "matched" pero NO son del catálogo → tratarlas como nuevas
  for (const k of Object.keys(matchedRaw || {})) {
    if (known.has(k) || k === "matched" || k === "tiers" || k === "new") continue;
    const p = toNum(matchedRaw[k]);
    if (p == null) continue;
    const name = String(k).trim();
    if (name && !newModels.some((m) => m.name.toLowerCase() === name.toLowerCase())) newModels.push({ name, cat: "", dept: "", price: p });
  }
...
export async function resolveSkuSmart({ catalog, catalogNames, apiKey }, name) {
  ...
  if (det) return det;
  try {
    const sys = "Mapeá el modelo del usuario al SKU EXACTO de esta lista, o null si no corresponde a ninguno. Respetá RAM/almacenamiento. Motorola EURO = los que terminan en 'EURO'; Motorola LATIN = los 'Motorola …' SIN 'EURO' (ej. 'G06 EURO' → 'Motorola G06 4+256 EURO'; 'G06 LATIN' → 'Motorola G06 4+256'). SOLO JSON {\"sku\": \"<exacto de la lista>\" o null}.\nLista:\n" + catalogNames.join("\n");
