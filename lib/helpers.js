...
// Variante regional: "US SPECS" / "USA SPEC" / "LATIN SPECS" es el MISMO teléfono que la versión
// genérica (regla del negocio) → se pliega en el genérico.
const REGIONAL_RE = /\b(?:us[a]?|latin)\s*specs?\b/i;
...
// pliega la variante regional "US/LATIN SPECS" en la genérica, y descarta TODO lo que va entre
// paréntesis (ruido del parser: cantidades "(100 pcs)"/"(1-20 pcs)", códigos "(SM-S947)"/"(F761)").
// CONSERVA el resto de los tokens (GB/DS/5G/color/capacidad) para no juntar productos distintos.
export function skuKey(name) {
  return String(name || "").toLowerCase().replace(REGIONAL_RE, "").replace(/\([^)]*\)/g, "").replace(/[^a-z0-9]/g, "");
}
