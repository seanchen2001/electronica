...
// trade: cotizado → confirmado → facturado → datos (color+IMEI, derivado) →
...
// ¿Todos los items tienen color e IMEI? (checkpoint "Datos", derivado — no se setea a mano)
function datosCompletos(items) {
  const list = items || [];
  if (!list.length) return false;
  return list.every((it) => String(it.color || "").trim() && String(it.imei || "").trim());
}
...
function invoiceTimeline(f, t, cli) {
  ...
  const steps = [
    ...
    { id: "datos", label: "Datos (color+IMEI)", done: datos, derivado: true },
  ];
...
export function tradeStatus({ drafts, invoiceHistory, opsTracking, clients }, ref) {
  ...
  for (const d of drafts || []) {
    ...
    const steps = [
      ...
      { id: "datos", label: "Datos (color+IMEI)", done: false, derivado: true },
