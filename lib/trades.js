...
function invoiceTimeline(f, t, cli) {
  ...
  const steps = [
    ...
    // IMEIs desactivados: se llevan por fuera de la app, así que no bloquean el trade
    { id: "datos", label: "IMEIs (por unidad)", done: datos, derivado: true, skipped: true },
  ];
...
export function tradeStatus({ drafts, invoiceHistory, opsTracking, clients }, ref) {
  ...
  for (const d of drafts || []) {
    ...
    const steps = [
      ...
      { id: "datos", label: "IMEIs (por unidad)", done: false, derivado: true, skipped: true },
