...
//  - Mac/MacBook/iMac → cat "Mac" en el dept "Apple" (y saca la categoría de teléfono si la tenía).
//  - iPhone fuera del dept "Apple": con precio → se mueve a Apple; variante de color SIN precio
...
const PHONE_CATS = new Set(["Samsung", "Samsung alta gama", "Samsung baja gama", "Motorola LATIN", "Motorola EURO", "Xiaomi y Redmi"]);
...
export function fixDepartments({ catalog = [], prices = {}, times = {}, tiers = {}, lista = {}, hiddenModels = [] }) {
  ...
  for (const c of catalog) {
    const dept = c.dept || "Otros";
    if (isMac(c.name)) {
      const cat = PHONE_CATS.has(c.cat) || !c.cat ? "Mac" : c.cat;
      if (dept === "Apple" && c.cat === cat) { out.push(c); continue; }
      out.push({ ...c, dept: "Apple", cat }); touched = true; continue;
    }
    if (isIphone(c.name) && dept !== "Apple") {
      if (!hasPrice(prices, c.name) && COLOR_RE.test(c.name)) { deleted.push(c.name); touched = true; continue; }
      out.push({ ...c, dept: "Apple", cat: "iPhone" }); touched = true; continue;
    }
    out.push(c);
  }
