...
export const AGENT_TOOLS = [
  {
    function_declarations: [
      ...
      {
        name: "switch_order",
        description: "Cambia el pedido ACTIVO al que corresponde al cliente (o id) indicado. Todo lo que hagas después es sobre ese pedido.",
        parameters: { type: "OBJECT", properties: { clientName: { type: "STRING" }, id: { type: "STRING" } } },
      },
      {
        name: "new_order",
...
export function buildAgentSystem({ catalogNames, suppliers, clientNames }) {
  return [
    ...
    "- COSTO negociado: si un proveedor te baja el COSTO de ESTA compra (no el precio de lista general), actualizá el costo de esas líneas con add_order_line/set_order_items pasando 'cost'. El precio al cliente NO cambia (mejora nuestro margen). Cambiar el precio de LISTA de un proveedor en general va por 'Cargar precios' (todavía no en esta etapa).",
