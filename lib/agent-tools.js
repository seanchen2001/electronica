...
export const AGENT_TOOLS = [
  {
    function_declarations: [
      ...
      {
        name: "load_imeis",
        description: "Carga los IMEIs (uno por unidad) a una factura, AGRUPADOS POR MODELO. Usalo cuando el usuario pega un bloque de IMEIs para una factura. Pasá invoiceNo y assignments: por cada modelo/línea, la lista de IMEIs. Se matchea el 'model' con la línea de la factura. Devuelve cuántos cargó por línea (cargados/esperados) y si quedó completo.",
        parameters: {
          type: "OBJECT",
          properties: {
...
export function buildAgentSystem({ catalogNames, suppliers, clientNames, shippingNames, learned }) {
  return [
    ...
    "  IMEIs (uno por unidad): NO pidas color — el color ya está en la factura. FLUJO cuando te pasan IMEIs para una factura #N: 1) llamá invoice_items(#N) para ver las líneas (modelo, color, cantidad). 2) Matcheá los IMEIs por MODELO y por CANTIDAD: si te pasan 50 IMEIs y hay una línea de 50 unidades, van a esa; si el modelo/color es obvio, usalo. 3) Si vienen en una IMAGEN, leé los IMEIs de la imagen. 4) Cargá con load_imeis(invoiceNo, assignments:[{model, imeis}]). 5) Confirmá cargados/esperados por línea. NO uses order_summary para esto (esa es la orden en armado, no la factura). Solo preguntá si de verdad es ambiguo (dos líneas con la misma cantidad y no se sabe cuál).",
