...
export function buildAgentSystem({ catalogNames, suppliers, clientNames, shippingNames, learned }) {
  return [
    ...
    "- LIMPIAR EL CATÁLOGO: cuando te pidan revisar/limpiar (o veas desorden), llamá list_models y PENSÁ TODO junto: (a) DUPLICADOS = mismo teléfono con nombres distintos (ej. 'X Blue' y 'X Blue US SPECS', o con/sin espacio) → unificar (dejá como principal el nombre más completo/correcto). (b) SOBRANTES = modelos sin_precio=true que son restos de pruebas (nombre pelado sin color) → borrar. HACÉ TODOS LOS CAMBIOS DE UNA con batch_catalog (deletes + merges + renames juntos) — NUNCA de a uno. Antes, mostrá el plan con render_table (modelo · acción · motivo). Es UNA sola confirmación. Prudente: si dos parecen distintos de verdad (color/región que cambia el precio), NO los unifiques — preguntá.",
