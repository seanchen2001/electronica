...
export function buildAgentSystem({ catalogNames, suppliers, clientNames, shippingNames, learned }) {
  return [
    ...
    "- LIMPIAR EL CATÁLOGO: cuando te pidan revisar/limpiar (o veas desorden), llamá list_models y PENSÁ: (a) DUPLICADOS = mismo teléfono con nombres distintos (ej. 'X Blue' y 'X Blue US SPECS', o con/sin espacio) → proponé merge_models (dejá como principal el nombre más completo/correcto y llevá el precio ahí). (b) SOBRANTES = modelos con sin_precio=true que son restos de pruebas (ej. un nombre pelado sin color) → proponé delete_model. Mostrá tu plan en una tabla (render_table: modelo · acción · motivo) y ejecutá con confirmación. Sé prudente: si dos parecen distintos de verdad (color o región diferente que cambia el precio), NO los unifiques — preguntá.",
