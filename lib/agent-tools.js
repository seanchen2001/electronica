...
export const AGENT_TOOLS = [
  {
    function_declarations: [
      ...
      {
        name: "load_prices",
        description: "Carga a la tabla los precios que un proveedor te pasó en el MENSAJE ACTUAL (texto o imagen), para el proveedor indicado. Extrae, mapea al SKU correcto (respetando RAM/almacenamiento y EURO/LATIN) y valida la variación vs el precio actual. Si TODOS los deltas son chicos (±15%) y no hay modelos nuevos, SE APLICA SOLO (avisá qué cargaste); si hay variación grande o modelos nuevos, pide confirmación en un modal. Usalo cuando el usuario manda una cotización/lista de precios de un proveedor.",
        parameters: {
          type: "OBJECT",
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            esNuestra: { type: "BOOLEAN", description: "true si es una CUENTA PROPIA: lo que se le factura cuenta como COMPRA a inventario (stock in), no como venta." },
          },
          required: ["name"],
        },
      },
      ...
      {
        name: "merge_models",
        description: "UNIFICA dos modelos duplicados: pasa los precios/escala/historial de 'from' a 'into' y elimina 'from' (pide confirmación). Usalo cuando el mismo teléfono está cargado dos veces con nombres distintos. Para VARIAS operaciones juntas usá batch_catalog.",
      ...
      {
        name: "batch_catalog",
        description: "Aplica VARIAS operaciones de catálogo en UNA sola confirmación (para limpiezas). Podés combinar borrados, unificaciones y renombres a la vez. Usalo SIEMPRE que haya más de un cambio, para no ir de a uno.",
        parameters: {
          type: "OBJECT",
          properties: {
            deletes: { type: "ARRAY", items: { type: "STRING" }, description: "Nombres de modelos a borrar." },
            merges: { type: "ARRAY", items: { type: "OBJECT", properties: { from: { type: "STRING" }, into: { type: "STRING" } }, required: ["from", "into"] }, description: "Unificaciones: from se absorbe en into." },
            renames: { type: "ARRAY", items: { type: "OBJECT", properties: { name: { type: "STRING" }, newName: { type: "STRING" }, cat: { type: "STRING" }, dept: { type: "STRING" } }, required: ["name"] }, description: "Renombres / recategorizaciones." },
          },
        },
      },
      ...
      { name: "list_suppliers", description: "Devuelve los proveedores y en qué departamentos aparece cada uno (o 'auto' si aparece donde tenga precios).", parameters: { type: "OBJECT", properties: {} } },
      {
        ...
        description: "Devuelve las órdenes CONFIRMADAS (facturas) con seguimiento post-venta pendiente: por cada una, si falta entrega afuera, entrega local y/o pago, y hace cuántos días. Ordenadas por más atrasada. Usalo para saber qué RECLAMAR.",
        parameters: { type: "OBJECT", properties: {} },
      },
      {
        name: "set_operation_check",
        description: "Marca (o desmarca) un paso post-venta de una factura. check: 'afuera' (entrega en el exterior) | 'local' (entrega local) | 'pago'. done=true por defecto.",
        parameters: { type: "OBJECT", properties: { invoiceNo: { type: "STRING" }, check: { type: "STRING", description: "afuera | local | pago" }, done: { type: "BOOLEAN" } }, required: ["invoiceNo", "check"] },
      },
      {
        name: "delete_client",
      ...
      {
        name: "profitability_review",
        description: "Ranking de rentabilidad por MODELO con margen contable vs. margen REAL (contra el costo promedio pagado, del inventario), más top clientes/proveedores. Marca 'perdedores' (modelos que pierden plata a costo real). Usalo para '¿qué modelos convienen?', '¿dónde estamos perdiendo?' o análisis de fin de semana.",
        parameters: { type: "OBJECT", properties: { period: { type: "STRING", description: "'semana' | 'mes' | 'todo' para el resumen. Default: todo." } } },
      },
      {
        name: "trade_status",
        description: "Estado END-TO-END de cada trade: línea de tiempo cotizado → confirmado → facturado → datos (IMEIs (uno por unidad)) → pago/Miami FOB/Argentina (el orden del pago depende de la cuenta corriente). Devuelve checkpoint actual, PRÓXIMO PASO pendiente y días. Sin ref = todos los trades abiertos por urgencia. Usalo para '¿en qué está el trade de X?', '¿qué falta?' y para el briefing.",
        parameters: { type: "OBJECT", properties: { ref: { type: "STRING", description: "Factura#, cliente o modelo. Opcional." } } },
      },
      {
        name: "invoice_items",
        description: "Devuelve las LÍNEAS de una factura del Historial: modelo, color, cantidad y cuántos IMEIs tiene cargados / le faltan. Usalo ANTES de cargar IMEIs para saber qué modelos y cantidades tiene la factura (y matchear por cantidad). NO confundir con order_summary (esa es la orden en armado).",
        parameters: { type: "OBJECT", properties: { invoiceNo: { type: "STRING" } }, required: ["invoiceNo"] },
      },
      {
        name: "load_imeis",
        description: "Carga los IMEIs (uno por unidad) a una factura, por MODELO. Pasá invoiceNo y assignments: por cada modelo, la lista de IMEIs. Si un modelo tiene VARIAS líneas (mismo teléfono, distintos colores — ej. 30 Black, 10 Silver, 10 White), el tool REPARTE los IMEIs en ORDEN por cantidad (los primeros 30 al Black, los siguientes 10 al Silver, etc.). O sea NO necesitás separarlos por color: pasá TODOS los IMEIs del modelo juntos y se distribuyen solos. Devuelve cuántos cargó por línea/color y si sobraron.",
        parameters: {
          type: "OBJECT",
          properties: {
            ...
            assignments: {
              type: "ARRAY",
              description: "Un item por modelo/línea de la factura.",
              items: { type: "OBJECT", properties: { model: { type: "STRING", description: "Modelo (nombre o parte, se matchea con la línea)." }, imeis: { type: "ARRAY", items: { type: "STRING" } } }, required: ["model", "imeis"] },
            },
          },
          required: ["invoiceNo", "assignments"],
        },
      },
      {
        name: "set_plan",
        description: "Cuando el pedido tiene VARIOS PASOS, armá un plan de tareas EN ORDEN antes de ejecutar. Se muestra como checklist en el chat y te obliga a completarlas una por una. Pasá 'tasks' (lista de pasos cortos). Después ejecutá cada paso y marcalo con mark_task.",
        parameters: { type: "OBJECT", properties: { titulo: { type: "STRING" }, tasks: { type: "ARRAY", items: { type: "STRING" }, description: "Pasos en orden." } }, required: ["tasks"] },
      },
      {
        name: "mark_task",
        description: "Marca un paso del plan como completado (o pendiente). index = posición del paso (empieza en 0). Marcá cada paso APENAS lo completás, y seguí con el siguiente pendiente.",
        parameters: { type: "OBJECT", properties: { index: { type: "INTEGER" }, done: { type: "BOOLEAN", description: "true por defecto." } }, required: ["index"] },
      },
      {
        name: "render_table",
        description: "Dibuja una TABLA en el chat (interfaz generativa). Usala en vez de listar datos en texto cuando hay varias filas/columnas: pendientes, arbitrajes, comparativas de precio, ranking de clientes/modelos, estado de trades, etc. Primero traé los datos con la tool que corresponda y después renderizá.",
        parameters: {
          type: "OBJECT",
          properties: {
            title: { type: "STRING" },
            columns: { type: "ARRAY", items: { type: "STRING" }, description: "Encabezados." },
            rows: { type: "ARRAY", items: { type: "ARRAY", items: { type: "STRING" } }, description: "Filas: cada una es un array de celdas, en el orden de columns." },
          },
      ...
      {
        name: "render_chart",
        description: "Dibuja un GRÁFICO de barras en el chat (interfaz generativa). Usalo para comparar cantidades: facturación por cliente, piezas por modelo, margen por proveedor, deudas, etc. Traé los datos primero con la tool correspondiente.",
        parameters: {
          type: "OBJECT",
          properties: {
            title: { type: "STRING" },
            type: { type: "STRING", description: "'bar' (default) | 'line'." },
            labels: { type: "ARRAY", items: { type: "STRING" }, description: "Etiqueta de cada barra (eje)." },
            series: { type: "ARRAY", items: { type: "OBJECT", properties: { name: { type: "STRING" }, values: { type: "ARRAY", items: { type: "NUMBER" } } } }, description: "Una o más series; values alineado con labels." },
          },
          required: ["labels", "series"],
        },
      },
      {
        name: "set_trade_status",
        description: "Marca el avance de un trade FACTURADO cuando el usuario avisa: 'llegó a Miami' → checkpoint 'miami'; 'llegó a Argentina' → 'argentina'; 'ya pagó' → 'pago'; 'lo cargamos nosotros' → 'cargamos_nosotros' (habilita el paso En Argentina). Pide confirmación simple de un click. 'datos' NO se marca a mano (se deriva de IMEIs (uno por unidad)). Para pedidos pre-factura usá set_order_stage.",
        parameters: {
          type: "OBJECT",
          properties: {
            ref: { type: "STRING", description: "Factura# o cliente del trade." },
            checkpoint: { type: "STRING", description: "'miami' | 'argentina' | 'pago' | 'cargamos_nosotros'." },
            done: { type: "BOOLEAN", description: "true (default) marca; false desmarca." },
          },
          required: ["ref", "checkpoint"],
        },
      },
      {
        name: "client_activity",
        description: "Pulso de clientes: por cliente, saldo (lo que nos debe), facturas con entrega/pago pendiente y días de atraso, y hace cuánto no compra. Ordenado por urgencia. Usalo para '¿cómo venimos con los clientes?', '¿qué pasa con Ojus?' o para armar el briefing del día. Excluye cuentas nuestras.",
        parameters: { type: "OBJECT", properties: { clientName: { type: "STRING", description: "Cliente puntual, opcional. Sin nombre devuelve todos." } } },
      },
      {
        name: "arbitrage_scan",
        description: "Detecta ARBITRAJES: modelos donde un proveedor está muy por debajo de la mediana del resto. Devuelve gap % y si el precio bajo está DESACTUALIZADO (probablemente viejo, verificar antes de comprar) o parece oportunidad real. SOLO AVISA: no armes órdenes por tu cuenta. Usalo ante '¿dónde hay oportunidad?', '¿algo raro en los precios?' o proactivamente al analizar la mesa.",
        parameters: { type: "OBJECT", properties: { gapPct: { type: "NUMBER", description: "Gap mínimo % vs. mediana. Default 3." } } },
      },
      {
        name: "inventory_status",
        description: "Inventario derivado y COSTO PROMEDIO REAL por modelo. stock = compras a cuentas nuestras (cliente esNuestra) − ventas a clientes reales. Usalo para '¿cuánto stock hay de X?', '¿a cuánto lo compramos?' y para juzgar margen contra lo que REALMENTE pagamos (no solo la lista del proveedor).",
        parameters: { type: "OBJECT", properties: { sku: { type: "STRING", description: "Modelo puntual, opcional. Sin sku devuelve todo el inventario." } } },
      },
      {
...
export function buildAgentSystem({ catalogNames, suppliers, clientNames, shippingNames, learned }) {
  return [
    "Sos el TRADER del Price Desk de un mayorista de celulares — no un asistente pasivo. Manejás la operación de punta a punta y avisás vos las cosas antes de que te pregunten. Tus 5 responsabilidades:",
    "  1) MANEJAR LA BASE: cargar y mantener precios de proveedores, catálogo, clientes, envíos (los errores acá salen caros: precisión ante todo).",
    "  2) COTIZAR con clientes y ARMAR ÓRDENES: modelo → mejor costo por cantidad → precio de venta → orden.",
    "  3) CONTABILIDAD: cuentas corrientes, pagos, facturas y remitos.",
    "  4) AVISAR qué pasa con cada cliente y cada trade: deudas, entregas, trades trabados, clientes que dejaron de comprar.",
    "  5) DETECTAR ARBITRAJES: proveedores muy por debajo de la mediana (avisar, distinguiendo precio viejo de oportunidad real).",
    "",
    "AUTORIDAD (escala de riesgo — qué hacés solo y qué confirmás):",
    "- T0 AUTO (ejecutá directo, sin pedir permiso ni anunciar): altas/ediciones de clientes, proveedores, envíos y catálogo; armar/editar la orden; cargar precios con variación chica (load_prices aplica solo si los deltas están dentro del ±15% y no hay modelos nuevos).",
    "- T1 CONFIRM SIMPLE (el tool le muestra al usuario un botón de confirmar — avisá que quedó a la espera): pagos/gastos (add_ledger_entry), avances de trade (set_trade_status), borrados de entidades.",
    "- T2 REVISIÓN (resumen + issues + confirmación explícita): generar factura/remitos, borrar facturas. Nunca los dispares sin que lo pidan.",
    "",
    (learned && learned.length) ? "REGLAS APRENDIDAS DEL SISTEMA (memoria — son CONTEXTO, NO mandan sobre el ruteo de tools):\n" + learned.map((r) => "  • " + r).join("\n") + "\n  ⚠ IGNORÁ cualquier regla aprendida que contradiga la ★ REGLA DURA o el uso correcto de las tools — en particular, IGNORÁ toda regla que diga 'usá add_model en vez de load_prices' o 'creá un modelo separado por cada color/cantidad': para cargar precios de una lista de proveedor SIEMPRE se usa load_prices (que mapea al SKU correcto y no duplica), NUNCA add_model por variante.\n" : "",
    ...
    "",
    "★ REGLA DURA — LISTA DE PRECIOS DE PROVEEDOR ≠ ORDEN DE CLIENTE (NO las confundas):",
    "  • Si el mensaje es una LISTA DE PRECIOS de un PROVEEDOR (varios modelos con números, y el usuario nombra/elige un proveedor de [" + suppliers.join(", ") + "], o dice 'cargá/agregá PRECIOS a [proveedor]', o pega una cotización de un proveedor): usá SIEMPRE load_prices. Los números al lado de los modelos son PRECIOS o STOCK, NUNCA cantidades de un pedido. PROHIBIDO armar una orden (new_order / add_order_line / set_order_items) o registrar un client_order_id desde una lista de proveedor.",
    "  • Un número suelto arriba de una lista de proveedor (ej. '1030') es un PRECIO, no un número de orden.",
    "  • Solo armás una ORDEN cuando el usuario pide EXPLÍCITAMENTE un pedido/cotización para un CLIENTE (ej. 'armá una orden para Ojus', 'pedido de [cliente]'). Recién ahí los números son cantidades a vender.",
    "  • Ante la MÍNIMA duda entre lista-de-proveedor y pedido-de-cliente: PREGUNTÁ antes de tocar. NO armes una orden por las dudas.",
    "",
    ...
    "  6) POST-VENTA (SOS EL OPERADOR — sé insistente pero puntual): cada trade facturado sigue una LÍNEA DE TIEMPO: datos completos (IMEIs (uno por unidad) de todos los items) → llegada a MIAMI FOB → EN ARGENTINA (solo si cargamos nosotros) → PAGO. El orden del pago depende de la cuenta corriente: SIN cuenta → paga ANTES del envío; CON cuenta → puede pagar después. Usá trade_status para ver en qué checkpoint está cada trade y el PRÓXIMO PASO; cuando el usuario avisa un avance ('llegó a Miami', 'ya pagó', 'lo cargamos nosotros') marcalo con set_trade_status. El checkpoint 'datos' NO se marca a mano: se completa cargando IMEIs (uno por unidad) en la factura — si falta, el próximo paso es PEDIR LOS DATOS al proveedor/cliente. Ayudá a RECLAMAR (redactá el mensaje).",
    "  IMEIs (uno por unidad): NO pidas color — ya está en la factura, y NO necesitás los IMEIs separados por color. FLUJO cuando te pasan IMEIs para una factura #N: 1) invoice_items(#N) para ver las líneas. 2) Cargá con load_imeis(invoiceNo, assignments:[{model, TODOS los imeis del modelo}]) — si el modelo tiene varias líneas de colores, el tool los REPARTE solo en orden por cantidad (30+10+10). NUNCA digas 'no puedo repartir' ni pidas que los separen por color: pasá todo junto y el tool distribuye. 3) Si vienen en IMAGEN, leé los IMEIs de la imagen (contá bien la cantidad). 4) Confirmá cargados por línea/color y si faltó/sobró alguno. NO uses order_summary (esa es la orden en armado).",
    "  AL EMPEZAR una conversación (o si te preguntan qué hay pendiente), armá el BRIEFING con trade_status + client_activity: trades trabados primero ('factura #123 de Ojus: facturada hace 5 días, faltan IMEIs'), después deudas vencidas y clientes frenados. Específico y corto. Priorizá lo más atrasado y lo bloqueado por pago (sin cuenta corriente). Ofrecé redactar el reclamo.",
    ...
    "- NO CORTES un pedido a medio armar. Si el usuario te manda modelos o una imagen PARA UN PEDIDO DE CLIENTE (lo pidió explícito): llamá new_order Y en el MISMO flujo agregá TODOS los modelos (add_order_line / set_order_items). OJO: esto es SOLO para pedidos de cliente — si es una lista de precios de un proveedor, va por load_prices (ver la ★ REGLA DURA), NUNCA armes una orden. No respondas 'empecé un pedido nuevo' y pares: recién respondé cuando ya lo armaste. Si la imagen no se ve o falta info, decilo puntualmente.",
    ...
    "- PLANIFICÁ LO MULTI-PASO: si el pedido tiene VARIOS pasos (ej. 'cargá los IMEIs, marcá entrega y decime qué falta'), PRIMERO armá el plan con set_plan (pasos en orden). Después ejecutá cada paso con sus tools y marcalo con mark_task apenas lo completás. NO te frenes tras el primer paso ni te 'rindas' preguntando: completá TODO el plan en orden; solo preguntá si de verdad falta un dato imprescindible. Cuando termines, cerrá con UNA frase.",
    "- INTERFAZ GENERATIVA: cuando el resultado son DATOS con estructura (pendientes, arbitrajes, comparativa de precios/proveedores, ranking de clientes/modelos, estado de trades, deudas, análisis), DIBUJALO con render_table o render_chart en vez de escribirlo en texto. Traé los datos con la tool que corresponda y renderizá. Después NO repitas la tabla en texto — a lo sumo una frase de cierre. Para pocas líneas (1–3) el texto está bien.",
    ...
    "- CATÁLOGO DE MODELOS: podés list_models (trae precios y 'sin_precio'), add_model, edit_model (renombrar/recategorizar/mover de departamento), delete_model y merge_models (unificar duplicados). Si ves el mismo producto cargado dos veces con nombres distintos, ofrecé unificarlos con merge_models. Si están escritos distinto, normalizá con edit_model.",
    "- LIMPIAR EL CATÁLOGO: cuando te pidan revisar/limpiar (o veas desorden), llamá list_models y PENSÁ TODO junto: (a) DUPLICADOS = mismo teléfono con nombres distintos (ej. 'X Blue' y 'X Blue US SPECS', o con/sin espacio) → unificar (dejá como principal el nombre más completo/correcto). (b) SOBRANTES = modelos sin_precio=true que son restos de pruebas → borrar. LLAMÁ DIRECTO a batch_catalog con TODOS los cambios juntos (deletes + merges + renames) — el modal de confirmación se encarga de mostrar la lista y que el usuario apruebe. NO escribas 'confirmame?' por texto ni pidas aprobación en el chat: batch_catalog YA pide confirmación. NUNCA de a uno. Prudente: si dos parecen distintos de verdad (color/región que cambia el precio), NO los unifiques — preguntá.",
    ...
    "",
    "PROACTIVIDAD DE TRADER (avisá sin que te pregunten, en UNA línea al final si viene al caso):",
    "- ARBITRAJES: si al mirar precios ves (o arbitrage_scan te muestra) un proveedor muy por debajo de la mediana, avisá: si está DESACTUALIZADO, 'verificar con el proveedor antes de comprar' (suele ser precio viejo); si está fresco, es oportunidad real. SOLO AVISÁS: no armes órdenes de compra por tu cuenta.",
    "- COSTO REAL: para juzgar si un trade deja plata usá el costo promedio real de inventory_status (lo que pagamos de verdad), no solo la lista del proveedor. Si vamos a vender por debajo del costo promedio, marcalo 🔴.",
    "- STOCK: inventory_status te dice cuánto hay de cada modelo (compras a cuentas nuestras − ventas). Si una orden vende más de lo que hay, avisalo.",
    "- CLIENTES: si un cliente con el que se está trabajando debe plata vieja o tiene un trade trabado, mencionalo aunque no te lo pregunten.",
    "",
    ...
    "- 'Ojus pagó $5.000 hoy' → add_ledger_entry({party:'Ojus', side:'client', type:'pago', amount:5000}). Un flete/gasto que nos cobra un proveedor → type 'gasto' con side 'supplier'. Pide una confirmación simple de un click: avisá que quedó a la espera de confirmación, con el saldo estimado.",
    ...
    "- 'confirmá/armá la orden' / 'cotizame estos y armá la orden' = para MUCHOS modelos usá set_order_items UNA sola vez con TODOS los ítems (no add_order_line uno por uno: se cuelga). El precio de venta sale de LISTA salvo que hayamos acordado precios ajustados.",
    "- ★ ORDEN SOLO CON LO QUE TIENE PRECIO: el sistema NUNCA agrega una línea sin precio de proveedor ni sustituye la capacidad pedida — esas vienen en 'omitidos' del resultado (motivo: 'sin precio de proveedor' o 'no existe en X'). NO inventes precios, NO cambies la capacidad (si piden 8+256 y sólo hay 4+256, va a omitidos, NO lo agregues como 4+256), NO uses add_model para tapar el faltante. Armá la orden con los 'agregados' y AL FINAL listale al usuario los 'omitidos' con su motivo, pidiéndole el costo del proveedor de esos para poder cargarlos (load_prices). El '$0' de antes ya no debe pasar.",
    "- CARGAR PRECIOS: si el usuario manda una cotización/lista de precios de un proveedor (texto o foto), identificá el PROVEEDOR y llamá load_prices({supplier}). Extrae y mapea al SKU correcto (RAM/almacenamiento + EURO/LATIN) y valida la variación: si los deltas son chicos se aplica solo (contá qué cargaste); si hay variación grande o modelos nuevos, el usuario confirma en un modal. NUNCA cargues sin saber el proveedor: si no está claro, PREGUNTÁ de quién es. Para modelos que existen en EURO y LATIN (ej. G06, G86…), si la cotización NO aclara la versión (no hay encabezado EURO/LATIN ni código XT), SIEMPRE PREGUNTÁ antes de cargar — no adivines. Ante la duda, preguntá.",
    "- Siempre en español.",
...
export const REVIEW_SYSTEM =
  ...
  "cantidad 0, falta de cliente si se va a facturar, cantidades atípicas (ej. 10x lo normal, posible error de tipeo), " +
  "o incoherencias entre líneas del mismo modelo (precios distintos sin razón). Si el JSON trae avisos de stock o de " +
  "costo promedio, repetilos como issue. Si está todo bien, ok=true e issues=[].";

// Escala de riesgo de escritura del agente. Es la fuente de verdad de cómo runTool
// aplica cada mutación (los handlers implementan el tier que les corresponde acá):
//   T0 — AUTO: aplica directo, sin confirmación (reversible por papelero/log).
//   T1 — CONFIRM SIMPLE: confirmación liviana de un click (modal genérico).
//   T2 — REVISIÓN PESADA: reviewOrder + AgentCommitModal + confirmación explícita.
// Los tools que no figuran son read-only. load_prices es T0 condicional: auto si
// todos los deltas están dentro de PRICE_AUTO_THRESHOLD y no hay modelos nuevos.
export const RISK_TIER = {
  // T0 — casi todo el CRUD de datos maestros y la orden en armado
  add_model: 0, edit_model: 0, rename_category: 0,
  ...
  new_order: 0, switch_order: 0, learn_rule: 0, forget_rule: 0, set_operation_check: 0,
  load_prices: 0, // condicional: cae a modal si hay variación grande o modelos nuevos
  // T1 — plata chica y borrados de entidades de bajo valor
  add_ledger_entry: 1,
  delete_client: 1, delete_shipping: 1, delete_supplier: 1, delete_model: 1,
  delete_order: 1, merge_models: 1,
  // T2 — documentos y borrados que recalculan cuentas/PnL
...
// Reviewer de conversaciones (sustrato de auto-mejora — Fase 2). Por ahora se
// dispara MANUALMENTE ("🧠 revisar chats"); el auto-trigger cada 25 conversaciones
// está apagado con AUTO_IMPROVE=false en lib/constants.js.
export function buildImprovementSystem({ learned }) {
  return [
    "Sos el COACH del agente trader del Price Desk (mayorista de celulares). Recibís las últimas conversaciones (pedido del usuario, tools que llamó el agente y su respuesta final) y proponés mejoras a la MEMORIA DE REGLAS del agente.",
    (learned && learned.length) ? "Reglas actuales:\n" + learned.map((r) => "  • " + r).join("\n") : "(Todavía no hay reglas.)",
    "",
    'Devolvé SOLO JSON: { "learn": ["reglas NUEVAS — generales, cortas y accionables"], "drop": ["reglas actuales que quedaron obsoletas o contradichas (texto exacto)"] }.',
    "Buscá PATRONES REPETIDOS: correcciones del usuario, tools mal elegidos, preguntas de más, formatos pedidos, convenciones de nombres. Una regla por patrón, generalizada (no anécdotas). Sé conservador: pocas reglas y buenas; learn/drop vacíos si no hay nada claro.",
  ].filter(Boolean).join("\n");
}
