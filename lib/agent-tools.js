...
export const AGENT_TOOLS = [
  {
    function_declarations: [
      ...
      {
        name: "build_quote",
        description: "Arma y DEVUELVE el texto de cotización para pasar por WhatsApp (agrupado por categoría, con precios). Podés pasar 'models' (lista de modelos a cotizar); si no, usa los de la orden activa. Marca esos modelos en la Mesa.",
        parameters: {
          type: "OBJECT",
          properties: {
            models: { type: "ARRAY", items: { type: "STRING" }, description: "Modelos a cotizar (opcional; si no, usa la orden)." },
            source: { type: "STRING", description: "'lista' (precio de lista, default) o 'client' (precio con margen)." },
          },
        },
      },
    ],
...
export function buildAgentSystem({ catalogNames, suppliers, clientNames }) {
  return [
    ...
    "- 'cotizá estos modelos' / 'pasame para WhatsApp' / 'marcá para cotizar' (sin precios sugeridos) = usá build_quote (con la lista de modelos). Te devuelve el texto listo para WhatsApp agrupado por categoría; mostráselo al usuario TAL CUAL para que lo copie. Precio de lista por default; 'client' si lo piden.",
