...
export const AGENT_TOOLS = [
  {
    function_declarations: [
      ...
      { name: "list_models", description: "Devuelve todos los modelos del catálogo con su categoría.", parameters: { type: "OBJECT", properties: {} } },
      {
        name: "add_model",
        description: "Agrega un modelo al catálogo. Categoría: Samsung | Motorola LATIN | Motorola EURO. Poné el nombre en el mismo estilo que el resto (ej. 'S26 12+256 5G').",
        parameters: { type: "OBJECT", properties: { name: { type: "STRING" }, cat: { type: "STRING", description: "Samsung | Motorola LATIN | Motorola EURO" } }, required: ["name"] },
      },
      {
        name: "edit_model",
        description: "Modifica un modelo: renombralo (newName) y/o cambiale la categoría (cat). Al renombrar se le llevan los precios/escala/lista/historial al nombre nuevo. Útil para normalizar nombres que quedaron distintos (ej. dos A17 escritos diferente). Si el nombre nuevo ya existe, usá merge_models.",
        parameters: { type: "OBJECT", properties: { name: { type: "STRING", description: "Nombre actual del modelo." }, newName: { type: "STRING", description: "Nombre nuevo (opcional)." }, cat: { type: "STRING", description: "Categoría nueva (opcional)." } }, required: ["name"] },
      },
      {
        name: "delete_model",
      ...
      { name: "list_suppliers", description: "Devuelve la lista de proveedores.", parameters: { type: "OBJECT", properties: {} } },
      {
        name: "delete_client",
...
export function buildAgentSystem({ catalogNames, suppliers, clientNames, shippingNames }) {
  return [
    ...
    "- CATÁLOGO DE MODELOS: podés list_models, add_model, edit_model (renombrar/recategorizar), delete_model y merge_models (unificar duplicados). Si ves el mismo teléfono cargado dos veces con nombres distintos, ofrecé unificarlos con merge_models. Para que dos variantes queden juntas en la tabla, el nombre debe ser igual salvo el 'RAM+almacenamiento' (ej. 'A17 4+128 DS' y 'A17 4+256 DS' → mismo modelo 'A17'). Si están escritos distinto, normalizá con edit_model.",
    "",
