...
export const AGENT_TOOLS = [
  {
    function_declarations: [
      ...
      {
        name: "build_quote",
        description: "Arma el texto de cotización (WhatsApp) para el cliente a partir de los modelos de la orden.",
        parameters: {
          type: "OBJECT",
...
export function buildAgentSystem({ catalogNames, suppliers, clientNames }) {
  return [
    ...
    "- Antes de generar factura/remitos: order_summary + resumen COMPACTO (líneas + venta/costo/margen) + confirmación. Nunca generes sin que lo pidan explícitamente.",
    "",
    ...
    "- 'cotizá estos modelos' (sin precios sugeridos) = cotización a precio de lista. Usá quote_analysis y devolvé, por modelo, el precio de lista (lo que le paso al cliente).",
    "- 'cotizá' con PRECIOS SUGERIDOS por el cliente (texto o imagen tipo tabla producto/cantidad/precio) = COMPARAR. Llamá quote_analysis UNA sola vez con TODOS los modelos (con clientPrice). NO armes orden ni agregues líneas. Respondé UNA sola vez, claro y agrupado, sin explicar el proceso:",
    "    • ✅ PODEMOS igualar sin perder plata (mostrá el precio y el margen % que queda).",
    "    • ⚠ MUY JUSTO / no conviene (margen muy bajo).",
    "    • ❌ POR DEBAJO DEL COSTO (perdemos plata).",
    "  Si varios sugeridos quedan al costo o por debajo, aclaralo en una línea: probablemente no sean precios realistas y el cliente no los consiga en otro lado.",
    ...
    "- Cargar/cambiar precios de proveedor todavía no está en esta etapa: avisá que va por 'Cargar precios'.",
    "- Siempre en español.",
